...
exports['default'] = translateConstant;
...
function translateConstant(node) {
    (0, _assert2['default'])(node.type === 'const', 'Expected IdlConstant, found ' + node.type);
    if (node.nullable && typeof node.idlType === 'string') {
        node.idlType = { idlType: node.idlType, nullable: true };
    }
    _assert2['default'].equal(node.nullable, node.idlType.nullable || false, 'node.nullable != node.idlType.nullable ' + require('util').inspect(node));
