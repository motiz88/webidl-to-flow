...
import translateImplements from './implements';

export
default

function translateRoot(node: IdlRootDefinition): Rx.Observable {
    assert(node.type === 'interface' || node.type === 'class' || node.type === 'callback interface' || node.type === 'exception' || node.type === 'enum' || node.type === 'typedef' || node.type === 'callback' || node.type === 'dictionary' || node.type === 'implements'
